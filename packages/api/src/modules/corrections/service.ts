...
export async function createCorrection(ctx: Context, input: CreateCorrectionInput) {
  ...
  if (input.disputedEventId) {
    ...
    if (!event || event.employeeId !== input.employeeId) badRequest("Disputed event must belong to the employee");
  }
...
export async function updateCorrection(ctx: Context, input: UpdateCorrectionInput) {
  ...
  if (current.requestedBy !== requireSessionUser(ctx)) forbidden("Only the requester can change a correction");
