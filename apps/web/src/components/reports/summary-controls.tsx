...
export function SummaryControls({
  ...
  return (
    ...
    >
      {/* Period toggle + prev/next/current/date-jump: one cluster on mobile, and — via
          sm:contents — flattens back into the original flat flex-wrap row at sm and up. */}
      ...
      {/* Filters + export: a 2-col grid (branch/department) plus a search+export row on
          mobile; sm:contents on both sub-groups flattens everything back into the original
          single ml-auto flex-wrap row at sm and up. */}
      <div className="flex flex-col gap-3 sm:ml-auto sm:flex-row sm:flex-wrap sm:items-center">
        <div className="grid grid-cols-2 gap-3 sm:contents">
          <select
            className={`${attendanceSelectClass} w-full sm:w-auto`}
            value={branchId}
            aria-label="Branch"
        ...
        <div className="flex items-center gap-2 sm:contents">
          ...
          <Button
            size="sm"
            variant="outline"
            className="h-9 shrink-0 rounded-[9px] px-3"
