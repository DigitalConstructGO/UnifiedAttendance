...
const moduleMeta = {
  ...
  Attendance: {
    icon: CalendarCheck2,
    description: "Daily records and derived attendance days.",
    ...
    <div className="mx-auto w-full max-w-[1240px] space-y-6">
      <section className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h2 className="text-strong mt-1 font-heading text-2xl font-bold tracking-[-0.03em]">
            Your operations workspace
          </h2>
        </div>
        <p className="text-xs text-muted-foreground">
          Access reflects your assigned role and permissions.
        </p>
      </section>
      ...
      <section aria-labelledby="access-summary-title">
        ...
        <h3 id="access-summary-title" className="sr-only">
          Access summary
        </h3>
        <div className="grid gap-3 sm:grid-cols-3">
          <div className="rounded-[18px] bg-card p-5 shadow-[var(--shadow-card)] ring-1 ring-border/80">
            ...
            <div className="mt-5 flex items-end justify-between">
              <p className="text-strong font-numeric text-3xl font-bold">{modules.length}</p>
              <span className="grid size-9 place-items-center rounded-[11px] bg-workflow/10 text-workflow">
          ...
          <div className="rounded-[18px] bg-card p-5 shadow-[var(--shadow-card)] ring-1 ring-border/80">
            ...
            <div className="mt-5 flex items-end justify-between gap-3">
              <p className="text-strong truncate font-heading text-xl font-bold">{access.role}</p>
              <span className="grid size-9 shrink-0 place-items-center rounded-[11px] bg-info/10 text-info">
          ...
          <div className="rounded-[18px] bg-card p-5 shadow-[var(--shadow-card)] ring-1 ring-border/80">
            ...
            <div className="mt-5 flex items-end justify-between">
              <p className="text-strong font-heading text-xl font-bold">Active</p>
      ...
      <section aria-labelledby="modules-title">
        ...
        <h3 id="modules-title" className="sr-only">
          Available modules
        </h3>
            ...
            return (
              ...
              >
                ...
                <div className="mt-auto pt-5">
                  <h4 className="text-strong font-heading text-base font-bold">{module.label}</h4>
      ...
      <section className="grid gap-3 lg:grid-cols-[1.25fr_0.75fr]">
        <div className="rounded-[18px] bg-card p-5 shadow-[var(--shadow-card)] ring-1 ring-border/80">
          <div className="flex items-center justify-between gap-4">
            <div>
              <h3 className="text-strong font-heading text-base font-bold">Workspace foundation</h3>
              <p className="mt-1 text-xs text-muted-foreground">
                A suggested path through the modules available to you.
              </p>
            </div>
          ...
          <ol className="mt-5 grid gap-3 sm:grid-cols-3">
            {setupSteps.map((step, index) => (
              <li key={step.href}>
                <Link
                  href={step.href}
                  className="text-strong hover:text-strong flex min-h-11 items-center gap-3 rounded-[11px] bg-[var(--surface-subtle)] px-3 py-3 text-xs font-semibold hover:bg-muted focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring"
                >
                  <span className="grid size-6 shrink-0 place-items-center rounded-lg bg-card font-numeric text-[0.6875rem] text-muted-foreground ring-1 ring-border">
                    {index + 1}
                  </span>
                  Open {step.label}
                </Link>
        ...
        <div className="bg-sidebar-gradient rounded-[18px] p-5 text-sidebar-foreground shadow-[var(--shadow-menu)]">
          <Fingerprint className="size-5 text-primary" aria-hidden="true" />
          <h3 className="mt-5 font-heading text-base font-bold text-white">
            Source records stay trustworthy
          </h3>
          <p className="mt-2 text-xs leading-5 text-sidebar-foreground/65">
            Device events remain preserved while reviewed corrections are recorded beside them.
          </p>
        </div>
      </section>
    </div>
